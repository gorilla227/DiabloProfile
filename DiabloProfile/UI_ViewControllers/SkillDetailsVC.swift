...
class SkillDetailsVC: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let gameData = AppDelegate.gameData(),
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if isActiveSkill {
            if let cell = tableView.dequeueReusableCellWithIdentifier("ActiveSkillCell", forIndexPath: indexPath) as? SkillDetailsVC_ActiveSkillCell, let skill = skill {
                cell.configureCell(skill, classKey: classKey)
                return cell
            }
